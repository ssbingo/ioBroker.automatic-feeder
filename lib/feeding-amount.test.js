...
describe('feeding-amount: applyDailyCap', () => {
	it('limits grams to the cap when the cap is set', () => {
		expect(applyDailyCap(100, 60)).to.equal(60);
	});
	it('leaves grams unchanged when below the cap', () => {
		expect(applyDailyCap(40, 60)).to.equal(40);
	});
	it('treats null/0/negative cap as no limit', () => {
		expect(applyDailyCap(100, null)).to.equal(100);
		expect(applyDailyCap(100, 0)).to.equal(100);
		expect(applyDailyCap(100, -5)).to.equal(100);
	});
	it('passes through a null amount', () => {
		expect(applyDailyCap(null, 60)).to.equal(null);
	});
});

describe('feeding-amount: dispenseSeconds', () => {
	it('divides grams by the rate', () => {
		expect(dispenseSeconds(100, 5)).to.equal(20);
	});
	it('is null for a missing/zero rate or amount', () => {
		expect(dispenseSeconds(100, 0)).to.equal(null);
		expect(dispenseSeconds(100, null)).to.equal(null);
		expect(dispenseSeconds(0, 5)).to.equal(null);
		expect(dispenseSeconds(null, 5)).to.equal(null);
	});
});

describe('feeding-amount: perFeedingSeconds', () => {
	it('splits the daily seconds across the feedings', () => {
		expect(perFeedingSeconds(20, 4)).to.equal(5);
	});
	it('is null without daily seconds or feedings', () => {
		expect(perFeedingSeconds(null, 4)).to.equal(null);
		expect(perFeedingSeconds(20, 0)).to.equal(null);
	});
});
