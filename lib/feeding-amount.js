 ...
 * band. Below 15 °C the manual gives no value → 0 % (the winter pause / temperature
 * block handle cold water). The temperature response is unimodal: it peaks in the
 * 23–28 °C optimum and is throttled again above it (appetite/oxygen). Bands (upper
 * bound inclusive): T<15, 15..18, 18..21, 21..23, 23..28, 28..30, >30.
 * `above23` therefore means "23..28 °C"; the field name is kept for compatibility.
 */
const PERCENT_DEFAULTS = { below15: 0, t15: 1, t18: 1.5, t21: 2, above23: 3, t28: 1.5, above30: 0.5 };
...
function feedPercentForTemp(tempC, tiers) {
	...
	if (t <= 28) {
		return nn(p.above23, 0); // 23–28 °C optimum
	}
	if (t <= 30) {
		return nn(p.t28, 0); // 28–30 °C throttle
	}
	return nn(p.above30, 0); // > 30 °C strong throttle
}
