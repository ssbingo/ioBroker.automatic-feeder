...
describe('lib/schedule', () => {
	...
	describe('pauseInfo / nextPauseBoundary (absolute feeding pauses)', () => {
		const h = 3600000;
		const ranges = [
			{ start: 100 * h, end: 110 * h },
			{ start: 200 * h, end: 205 * h },
		];

		it('reports active with the correct resume time inside a pause', () => {
			const r = pauseInfo(ranges, 105 * h);
			expect(r.active).to.equal(true);
			expect(r.until).to.equal(110 * h);
		});
		it('is inactive outside any pause', () => {
			expect(pauseInfo(ranges, 150 * h).active).to.equal(false);
			expect(pauseInfo([], 150 * h).active).to.equal(false);
		});
		it('treats start as inclusive and end as exclusive', () => {
			expect(pauseInfo(ranges, 100 * h).active).to.equal(true); // at start
			expect(pauseInfo(ranges, 110 * h).active).to.equal(false); // at end
		});
		it('extends the resume time to the latest end for overlapping pauses', () => {
			const ov = [
				{ start: 10 * h, end: 20 * h },
				{ start: 15 * h, end: 30 * h },
			];
			expect(pauseInfo(ov, 16 * h).until).to.equal(30 * h);
		});
		it('nextPauseBoundary returns the next start or end after now', () => {
			expect(nextPauseBoundary(ranges, 50 * h)).to.equal(100 * h); // next start
			expect(nextPauseBoundary(ranges, 105 * h)).to.equal(110 * h); // current end
			expect(nextPauseBoundary(ranges, 210 * h)).to.equal(null); // nothing ahead
		});
	});
});
