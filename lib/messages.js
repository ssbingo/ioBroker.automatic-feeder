...
const MESSAGES = {
	...
	// {minutes} = the configured lead time (announceLeadMin); {unit} = the correctly
	// pluralized "minutes" word for that number and language (see minuteUnit()).
	announce: {
		en: 'Attention! The next feeding starts in {minutes} {unit}',
		de: 'Achtung! Die nächste Fütterung beginnt in {minutes} {unit}',
		ru: 'Внимание! Следующее кормление начнётся через {minutes} {unit}',
		pt: 'Atenção! A próxima alimentação começa em {minutes} {unit}',
		nl: 'Let op! De volgende voedering begint over {minutes} {unit}',
		fr: 'Attention ! La prochaine distribution commence dans {minutes} {unit}',
		it: 'Attenzione! La prossima alimentazione inizia tra {minutes} {unit}',
		es: '¡Atención! La próxima alimentación comienza en {minutes} {unit}',
		pl: 'Uwaga! Następne karmienie rozpocznie się za {minutes} {unit}',
		uk: 'Увага! Наступне годування почнеться через {minutes} {unit}',
		'zh-cn': '注意！下一次投喂将在 {minutes} {unit}后开始',
	},
...
/**
 * Returns the correctly pluralized "minutes" word for a count in a given language,
 * using the language's CLDR plural rules (Intl.PluralRules). Falls back to the "other"
 * form (and to English) when a form or the locale is unavailable.
 *
...
function minuteUnit(lang, n) {
	const forms = MINUTE_FORMS[lang] || MINUTE_FORMS[DEFAULT_LANGUAGE];
	let cat = 'other';
	try {
...
module.exports = { MESSAGES, SUPPORTED_LANGUAGES, DEFAULT_LANGUAGE, translate, minuteUnit };
