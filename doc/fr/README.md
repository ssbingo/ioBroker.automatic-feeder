...
* **Le déclencheur manuel ignore tous les blocages** — si activé, le bouton et les points de données
  `feedNow` / `feedFor` distribuent même en cas de blocage par température/fenêtre actif.
...
#### Modèle de quantité d'alimentation (indicatif)

En option, l'adaptateur estime la **quantité quotidienne de nourriture recommandée** pour un interrupteur à partir du **cheptel de poissons** et de la **température de l'eau**, conformément au manuel d'origine du distributeur : `daily amount [g] = total fish weight × percentage(water temperature)`. Vous saisissez uniquement le **nombre de poissons par classe de taille** (15/20/30/40/50/60 cm) dans un **petit tableau avec une icône de poisson** par taille ; le **poids par taille** est une **estimation fixe issue du manuel** (60/125/350/1000/2000/4000 g). Vous définissez également le **pourcentage d'alimentation par plage de température** (valeurs par défaut 0 % en dessous de 15 °C, 1 % à 15–18 °C, 1.5 % à 18–21 °C, 2 % à 21–23 °C, 3 % au-dessus de 23 °C). Il nécessite une **source de température de l'eau** pour l'interrupteur.

Il s'agit **uniquement d'un calculateur** – il calcule et affiche la recommandation mais ne **modifie pas** la façon dont ni le moment où l'interrupteur distribue. Les résultats sont publiés dans `status.fishTotalWeight` (g), `status.feedPercentToday` (%) et `status.feedTargetGramsToday` (g) ; l'onglet de l'interrupteur affiche en outre le poids total estimé et un exemple. (La distribution effective de cette quantité – la conversion des grammes en temps de fonctionnement via un débit de distributeur calibré – est prévue pour une étape ultérieure.)
...
> **Remarque :** tous les points de données horodatés sont affichés dans le **fuseau horaire local du système** (format `JJ.MM.AAAA HH:MM:SS`, p. ex. `01.07.2026 16:20:00`). Pour VIS et les scripts, chaque horodatage dispose en plus d'un **jumeau numérique** se terminant par `…Ts` (temps Unix en **millisecondes**, `0` = aucun) — idéal pour les comptes à rebours et les barres de temps sans aucune analyse de chaîne, et indépendant du format d'affichage.
...
| `status.nextFeedingTs` | number (ro) | Prochaine distribution planifiée en temps Unix en ms (`0` = rien de planifié). |
| `status.blocked` | boolean (ro) | La dernière tentative a été bloquée. |
| `status.blockReason` | string (ro) | Raison du blocage (nuit / température / oxygène), dans la langue du système. |
| `status.blockReasonCode` | string (ro) | La raison du blocage sous forme de **code stable lisible par machine** (p. ex. `blockNight`, `blockWaterBelow`, `blockPauseManual` ; vide = pas de blocage) — pour la logique d'icônes/de couleurs dans VIS, indépendamment de la langue. |
...
| `status.pauseActiveUntilTs` | number (ro) | Fin de la pause de distribution active en temps Unix en ms (`0` = aucune). |
...
| `status.feedTargetGramsToday` | number (ro) | Modèle de quantité d'alimentation : quantité de nourriture recommandée par jour (g). À titre indicatif uniquement – ne contrôle pas la distribution. |
...
* Ou utilise un curseur/champ numérique qui écrit les **secondes** sur
  `automatic-feeder.0.switches.sw-0.feedFor` → distribue **une fois avec exactement cette durée**
  (sans modification de la configuration, sans redémarrage ; l'état se réinitialise ensuite à `0`).
