...
przyciskami czasowymi i własnym interfejsem WWW). Dla każdego przełącznika osobno decydujesz, czy
korzysta on z takiej płytki; gdy włączysz ją dla danego przełącznika w ustawieniach podstawowych,
ten przełącznik zyskuje zakładkę **Przekaźnik**, w której ustawiasz adres sieciowy płytki,
...
|-------------|---------|
| **ioBroker** z **admin ≥ 7.8.23**, **js-controller ≥ 6.0.11** i **Node.js ≥ 22** | Wymagane wersje minimalne. Strona konfiguracji jest zrealizowana w React 19. |
...
**Przypomnienia o przerwie zimowej** (jeśli włączone, zobacz *Przerwa zimowa*) są wysyłane do tej
...
* **`settings`** (`switches.<id>.settings.*`) – **edytowalne** odzwierciedlenie konfiguracji tego
  ...
  (np. `winterWindow`).
* **`relay`** (`switches.<id>.relay.*`) – obecny tylko wtedy, gdy ten przełącznik używa płytki
...
**Przerwa zimowa dla stawu**
* W zakładce przełącznika otwórz *Przerwa zimowa*, włącz ją, ustaw *Początek zimy* `01.11` i
...
3. Zapisz. Od teraz wybrane wyniki monitorowania są wysyłane do Telegrama (poprzedzone nazwą
   ...
   przełącznika.
4. **Przypomnienia o przerwie zimowej** używają tej samej instancji Telegram i tego samego
   odbiorcy. Są sterowane w sekcji *Przerwa zimowa* (liczba dni przed rozpoczęciem/zakończeniem
   oraz godzina przypomnienia) i **nie** wymagają aktywowania monitorowania.
