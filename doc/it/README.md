...
Facoltativamente l'adattatore stima la **quantità giornaliera di mangime consigliata** per un interruttore a partire dalla **popolazione ittica** e dalla **temperatura dell'acqua**, seguendo il manuale originale del distributore di mangime: `daily amount [g] = total fish weight × percentage(water temperature)`. Inserisci solo il **numero di pesci per classe di taglia** (15/20/30/40/50/60 cm) in una **piccola tabella con un'icona a forma di pesce** per ogni taglia; il **peso per taglia** è una stima fissa dal manuale (60/125/350/1000/2000/4000 g). Imposta inoltre la **percentuale di alimentazione per fascia di temperatura** (valori predefiniti 0 % sotto i 15 °C, 1 % a 15–18 °C, 1,5 % a 18–21 °C, 2 % a 21–23 °C, 3 % a 23–28 °C, poi nuovamente ridotta con il caldo: 1,5 % a 28–30 °C e 0,5 % sopra i 30 °C – la risposta alla temperatura raggiunge il picco intorno ai 24–26 °C e diminuisce al di sopra). Richiede una **fonte di temperatura dell'acqua** per l'interruttore.

Si tratta di un **calcolatore**: calcola e mostra la raccomandazione. I risultati vengono pubblicati in `status.fishTotalWeight` (g), `status.feedPercentToday` (%) e `status.feedTargetGramsToday` (g); la scheda dell'interruttore mostra inoltre il peso totale stimato e un esempio.
...
| `status.feedingsPerDayToday` | number (ro) | Modello della quantità di mangime: numero di distribuzioni pianificate per oggi. |
