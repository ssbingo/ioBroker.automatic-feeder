...
> ### 🔌 Hardware compatível — a placa temporizadora **Feeder-Relais**
>
> Está a montar um alimentador e prefere usar uma placa temporizadora pronta em vez de ligar o seu próprio relé? Veja o **[Feeder-Relais (Timer-Ersatzplatine)](https://github.com/ssbingo/timer-ersatzplatine)** — uma placa temporizadora ESP32 de construção própria que combina perfeitamente com este adaptador ([visão geral online](https://ssbingo.github.io/timer-ersatzplatine/)).
>
> **É um projeto separado e autónomo — independente do adaptador.** A placa e o adaptador correspondem-se entre si, mas são totalmente independentes um do outro: o adaptador funciona plenamente sem a placa e a placa funciona sem o adaptador.
...
botões de temporizador e a sua própria interface web). Você decide **por interruptor** se ele
usa uma placa dessas; quando você a ativa para um interruptor nas configurações básicas, esse
interruptor recebe uma aba **Relé** onde você define o endereço de rede da placa, testa a ligação
e configura os seus três tempos de alimentação dos botões (S1–S3) diretamente a partir do adaptador.
...
|-------------|---------|
| **ioBroker** com **admin ≥ 7.8.23**, **js-controller ≥ 6.0.11** e **Node.js ≥ 22** | Versões mínimas exigidas. A página de configuração é implementada com React 19. |
...
Opcionalmente, o adaptador estima a **quantidade diária de ração recomendada** para um interruptor a partir da **população de peixes** e da **temperatura da água**, seguindo o manual original do alimentador: `daily amount [g] = total fish weight × percentage(water temperature)`. Você indica apenas o **número de peixes por classe de tamanho** (15/20/30/40/50/60 cm) numa pequena tabela com um ícone de peixe por tamanho; o **peso por tamanho** é uma estimativa fixa do manual (60/125/350/1000/2000/4000 g). Você também define a **percentagem de alimentação por faixa de temperatura** (padrões 0 % abaixo de 15 °C, 1 % entre 15–18 °C, 1,5 % entre 18–21 °C, 2 % entre 21–23 °C, 3 % entre 23–28 °C, depois novamente reduzida no calor: 1,5 % entre 28–30 °C e 0,5 % acima de 30 °C – a resposta à temperatura atinge o pico por volta de 24–26 °C e diminui acima disso). Precisa de uma **fonte de temperatura da água** para o interruptor.

Isto é **apenas uma calculadora** – calcula e mostra a recomendação. Os resultados são publicados em `status.fishTotalWeight` (g), `status.feedPercentToday` (%) e `status.feedTargetGramsToday` (g); a aba do interruptor mostra ainda o peso total estimado e um exemplo.
...
* **`settings`** (`switches.<id>.settings.*`) – um espelho **editável** da configuração deste
  ...
  (p. ex. `winterWindow`).
* **`relay`** (`switches.<id>.relay.*`) – presente apenas quando este interruptor usa uma placa de
  relé; os pontos de dados de status somente leitura da placa de relé listados no fim da tabela.
...
| `status.feedingsPerDayToday` | number (ro) | Modelo de quantidade de ração: número de alimentações planeadas para hoje. |
